...
public class UserPanel extends JPanel {
	...
	public void updateClippedImage(Graphics2D g2d, BufferedImage icon) {
		...
		BufferedImage img = gc.createCompatibleImage(PortalApp.USER_ICON_WIDTH,
				PortalApp.USER_ICON_HEIGHT, Transparency.TRANSLUCENT);
		...
		g2.drawImage(icon, 0, 0, PortalApp.USER_ICON_WIDTH,
				PortalApp.USER_ICON_HEIGHT, null);
		g2.dispose();
	...
	public void paintComponent(Graphics g) {
		...
		if (m_alpha < 1) {
			AlphaComposite ac = AlphaComposite.getInstance(
					AlphaComposite.SRC_OVER, Math.max(m_alpha, 0));
			g2d.setComposite(ac);
		}
		...
		if (icon == null)
			icon = PortalApp.DEFAULT_USER_ICON;
		...
		if (m_hoverCircleWidth > PortalApp.USER_ICON_WIDTH
			...
			g2d.fillOval(
					-(m_hoverCircleWidth - PortalApp.USER_ICON_WIDTH >> 1),
					-((m_hoverCircleHeight - PortalApp.USER_ICON_HEIGHT) >> 1),
					m_hoverCircleWidth, m_hoverCircleHeight);
		...
		if (m_hovering
				&& m_hoverCircleWidth < PortalApp.USER_ICON_WIDTH
						+ PortalApp.USER_ICON_HOVER_RADIUS) {
			m_hoverCircleWidth += PortalApp.USER_ICON_HOVER_SPEED;
		}
		if (m_hovering
				&& m_hoverCircleHeight < PortalApp.USER_ICON_HEIGHT
						+ PortalApp.USER_ICON_HOVER_RADIUS) {
	...
	public Dimension getMinimumSize() {
		return new Dimension(Math.max(PortalApp.USER_ICON_WIDTH,
				PortalApp.USER_MAX_NAME_WIDTH), PortalApp.USER_ICON_TOP_SPACE
				+ PortalApp.USER_ICON_HEIGHT + PortalApp.USER_NAME_SPACING
				+ PortalApp.USER_NAME_LINE_HEIGHT
	...
	/*
	 * public class FileDropHandler extends TransferHandler { private static
	 * final long serialVersionUID = 1L;
	 * 
	 * @Override public boolean canImport(TransferHandler.TransferSupport info)
	 * { info.getDropLocation().getDropPoint(); // we only import FileList if
	 * (!info.isDataFlavorSupported(DataFlavor.javaFileListFlavor)) { return
	 * false; } System.out.println("Hovering..."); m_hovering = true;
	 * 
	 * return true; }
	 * 
	 * @SuppressWarnings("unchecked")
	 ...
	 * 
	 * // Check for FileList flavor if
	 * (!info.isDataFlavorSupported(DataFlavor.javaFileListFlavor)) {
	 * displayDropLocation("List doesn't accept a drop of this type."); return
	 * false; }
	 * 
	 * // Get the fileList that is being dropped. Transferable t =
	 * info.getTransferable(); List<File> data; try { data =
	 * (List<File>)t.getTransferData(DataFlavor.javaFileListFlavor); } catch
